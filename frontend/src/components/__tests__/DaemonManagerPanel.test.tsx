...
// SessionInfo type matching wailsjs/go/main/App.d.ts (Phase 137 / SHARE-03 browseEnabled added)
...
const mockSessions: SessionInfo[] = [
  { id: 'sess-1', cli: 'claude', name: 'claude 1', state: 'running', status: 'running', createdAt: '2026-04-01T10:00:00Z', hostname: 'macbook-pro.local', webEnabled: false, viewerCount: 0, homeDir: false, browseEnabled: false, workDir: '' },
  { id: 'sess-2', cli: 'codex', name: 'codex 1', state: 'idle', status: 'idle', createdAt: '2026-04-01T11:00:00Z', hostname: 'dev-server.internal', webEnabled: false, viewerCount: 0, homeDir: false, browseEnabled: false, workDir: '' },
]
...
describe('DaemonManagerPanel (DMGR-03) - DOM tests', () => {
  ...
  it('renders em dash when hostname is empty', () => {
    const noHostSessions = [
      { id: 'sess-3', cli: 'claude', name: 'test', state: 'running', status: 'running', createdAt: '2026-04-01T12:00:00Z', hostname: '', webEnabled: false, viewerCount: 0, homeDir: false, browseEnabled: false, workDir: '' },
    ]
  ...
  it('renders "Enable file writes" toggle checked when session arrives with filesWrite=true', async () => {
    ...
    const writeSessions: SessionInfo[] = [
      { id: 'sess-fw', cli: 'claude', name: 'write-on session', state: 'running', status: 'running',
        ...
        homeDir: false, browseEnabled: true, workDir: '' },
    ]
    await act(async () => {
  ...
  it('renders "Enable file writes" toggle unchecked when session has filesWrite=false', async () => {
    const noWriteSessions: SessionInfo[] = [
      { id: 'sess-nw', cli: 'claude', name: 'write-off session', state: 'running', status: 'running',
        createdAt: '2026-04-01T10:00:00Z', hostname: 'host', webEnabled: false, viewerCount: 0,
        homeDir: false, browseEnabled: false, workDir: '' },
    ]
    await act(async () => {
...
describe('DaemonManagerPanel WR-04 \u2014 homeDir banner sourced from SessionInfo.homeDir', () => {
  ...
  it('shows home-dir banner when s.homeDir=true and writes are enabled via sessionWrites state', async () => {
    ...
    // We must mock IssueCapabilities and SetSessionBrowse for the toggle interaction.
    const { IssueCapabilities, SetSessionBrowse } = await import('../../wailsjs/go/main/App')
    const mockIssue = vi.mocked(IssueCapabilities)
    const mockSetWrite = vi.mocked(SetSessionBrowse)
    ...
    const homeDirSession: SessionInfo[] = [
      { id: 'sess-home', cli: 'claude', name: 'home session', state: 'running', status: 'running',
        ...
        homeDir: true, browseEnabled: false, workDir: '' },
    ]
