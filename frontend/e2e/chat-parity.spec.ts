...
test.describe('Phase 155 — chat parity gate', () => {
  ...
  test('ROCHAT-01 — RO viewer CAN post chat; ROCHAT-02 — @session inject stays gated', async ({ browser }) => {
    ...
    try {
      ...
      // WS connection warm-up: wait for any history message to confirm the WSS
      // handshake and history fetch completed. On Firefox/WebKit the TLS handshake
      // over the self-signed cert can exhaust an 8s budget before history renders.
      // Using .first() (any message) resolves quickly once the WS is up and is
