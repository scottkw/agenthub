...
- [x] **FIX-01**: Web-share guests on the `/app/` surface receive live plugin-config and SSE hot-swap updates (self-fetched via the capability-gated `/api/plugin-config` + SSE endpoints), restoring the parity lost after the Phase 159 `/sessions/{id}` → `/app/` redirect. — Issue #112
...
- [ ] **FIX-05** (re-scoped 2026-07-02): On non-admin macOS `macsys` accounts where `sameuserproof` is unreadable, Tailscale detection must report an **honest, accurate** state — distinguishing "daemon running but status unreadable due to OS permissions" from "installed but not connected" — instead of a false "not connected", with actionable user guidance (grant admin, or use the Homebrew build). — Issue #120
  - **Original approach (CLI `status` fallback) INVALIDATED.** Phase 169 code review (CR-01, confirmed via vendored `tailscale.com@v1.98.3` source + live `/Library/Tailscale/` check): `tailscaled` writes `sameuserproof` as `0640 root:admin` **by design** (`safesocket_darwin.go` `Fchown(...,0,80)`). Both the SDK read and a spawned non-setuid `tailscale` CLI run as the same OS user and hit the identical per-*user* permission gate; only the macsys GUI bypasses it via in-process `SetCredentials`. A same-user shell-out grants no extra privilege, so it cannot report true Connected status.
  - **New approach (chosen 2026-07-02): honest permission-aware detection** — detect `sameuserproof` present-but-`EACCES` (daemon running, status unreadable) as a distinct state from daemon-down; surface it accurately in the UI; revert 169-01's CLI-fallback code. No privileged helper, no false Connected=true.
...
| FNL-08 | Phase 170 | Done (M-46 live UAT PASSED 2026-07-06) |
