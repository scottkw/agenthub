...
- [x] **WEBCHAT-05**: A web-share guest cannot rename the session tab or invoke the desktop-only tab menu. The tab session-menu (Rename, Save Terminal As — both Wails RPCs with no browser bridge) and double-click/right-click rename are suppressed in web mode; only the × close button remains. (Rename never reached the host — the RPC fails silently and relabels only the local browser tab — but the affordance is removed for clarity. Gap-closure from 159 live UAT.)
