...
| **Owner `files.write` default** | **Opt-in for all tokens (never default-on)** | No cap token — owner or viewer — carries `files.write` by default. The owner explicitly enables write access per session (a per-session toggle, default OFF); web-share viewers require a further explicit grant on top of that. Matches the research recommendation: a home-directory session (Claude Code's default `cwd=$HOME`) is never one mis-share away from RC-file overwrite. The shell-RC denylist (FSW-06) and home-dir warning (CAP-06) remain as defense-in-depth. |
...
- [ ] **FSW-06** — Server-side sensitive-file denylist enforced inside ALL `Sandbox` write methods (write/rename/delete/mkdir): reject operations targeting shell RC files (`~/.bashrc`, `~/.zshrc`, `~/.profile`, `~/.bash_profile`), `~/.ssh/*`, `~/.claude/*`, and the daemon's own config dir, returning `403 Protected system file`. **Defense-in-depth** — `files.write` is opt-in, but AI sessions commonly run with `cwd=$HOME`, so once a user enables writes (or grants a viewer) the sandbox is technically correct yet the consequences of overwriting SSH keys / shell init are severe. The denylist backstops a conscious-but-careless grant.
...
Wires the write handler into `internal/webserver` behind a new capability bit and CSRF protection. The capability model mirrors v3.4's `files.read`/`requireFilesRead` exactly, with one ratified difference from the read side: `files.write` is never granted by default — the owner explicitly enables it per session (read defaulted on for the owner; write does not).
...
- [ ] **CAP-04** — `files.write` is opt-in for every token (ratified — never default-on). Neither the session-owner cap nor the web-share viewer cap includes `files.write` unless write access is explicitly enabled: a per-session "Enable file writes" toggle (default OFF) gates the owner's cap, and web-share viewers require a further explicit opt-in on top of an enabled session. (The local daemon socket remains auth-less per FSW-08; this gates the capability-bearing webserver + remote surfaces.)
...
- [ ] **CAP-06** — Home-directory write warning: when a user enables `files.write` for a session whose resolved cwd is `$HOME` (or a sensitive ancestor), the GUI and TUI surface a visible warning at the point of enabling that file writes can affect dotfiles, SSH keys, and shell config. Defense-in-depth alongside the FSW-06 denylist.
