...
- 🔄 **v1.13 Cross-Platform Fixes & UX** — Phases 67-69 (active)
...
**Goal**: Linux and Windows users can interact with AgentHub through a system tray icon that matches macOS tray functionality
**Depends on**: Nothing (independent of other v1.13 phases)
**GitHub issues**: #11 (System tray icons bugfix)
**Requirements**: TRAY-01, TRAY-02, TRAY-03, TRAY-04, TRAY-05, TRAY-06
**Success Criteria** (what must be TRUE):
  1. On Linux (GNOME/KDE/XFCE with AppIndicator or StatusNotifier support), an AgentHub tray icon appears in the system tray after app launch
  2. On Windows, an AgentHub tray icon appears in the notification area (system tray) after app launch
  3. Right-clicking the Linux or Windows tray icon shows a menu listing active sessions, matching the session list shown on macOS
...
**Goal**: AgentHub reliably finds agent CLIs and Tailscale on all platforms regardless of how they were installed, and macOS install instructions show a single copyable command
**Depends on**: Nothing (independent of other v1.13 phases)
**GitHub issues**: #14 (macOS install instructions), #15 (Application searching)
**Requirements**: DISC-01, DISC-02, DISC-03, INST-01
**Success Criteria** (what must be TRUE):
...
**Depends on**: Nothing (independent of other v1.13 phases)
**GitHub issues**: #16 (New Settings tab layout)
