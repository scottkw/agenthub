...
v1.14 shipped (2026-04-14). 15 milestones shipped (v1.0–v1.14), 78 phases completed, 151 plans total. Phase 78 (tui-remote-qr) complete — TUI surfaces remote tailnet peer sessions in a unified session list (grouped by peer hostname with `── Remote: {host} ── divider rows`, j/k/g/G navigation skips dividers, selection restores by identity after 2s refresh, header count shows "N local, M remote", kill/rename blocked on remotes with toast), and `q` opens a scannable ASCII QR overlay for the selected session's web URL using go-qrcode ToSmallString (quit reassigned to Q/Ctrl+C; 55×25 terminal size guard; per-session WebEnabled check so unserved sessions show a toast). Integration test `TestTUIRemoteAndQR_FullFlow` exercises the complete flow. UAT surfaced a WebEnabled bug inline (fixed in ef7b51f: sessionURL() now requires `entry.session.WebEnabled` alongside the global web-server gate). 77 TUI tests pass, full project suite green across 11 packages. Requirements satisfied: TUI-07, TUI-10. This completes milestone v2.0. Phase 77 (tui-session-operations) complete — full session lifecycle from TUI: attach (Enter → tea.Exec → raw PTY with status bar → Ctrl-\ resume via `attachDoneMsg`), new-session modal (agent picker with Left/Right cycling, directory pre-filled with cwd, args textinput, Tab focus cycling, submit validation), kill confirmation (bordered danger-styled modal with default-No safety, y/n/Esc/Enter toggle), and inline rename (r replaces name cell with textinput, Enter commits, Esc cancels, empty/same-name rejected). Priority-based key dispatch (editing > kill confirm > new session modal > help > main). Shared attach logic extracted into new `internal/attach/` package with ANSI-safe border-title injection, allowlist attach-status guard, and error-propagating `AttachSession`. 57 unit tests pass across `internal/tui` and `internal/attach`. All 4 human UAT items verified by user (attach/new/kill/rename). Phase 76 (tui-foundation) complete — `agenthub tui` launches a Bubble Tea interactive terminal UI (Charm v2: bubbletea/v2, lipgloss/v2, bubbles/v2) showing a live session list with heuristic status glyphs, web server status pill, discoverable `?` help overlay, adaptive color themes via tea.BackgroundColorMsg, graceful sub-60×10 fallback, non-TTY stdout guard with exit 1, and 2s tick refresh via tea.Cmd. New `internal/tui` package built on daemon client types; `SessionInfo.Status` enriched from heuristic detector. Phase 75 (cli-status-bar) complete — `agenthub attach` shows a persistent DECSTBM scroll-region status bar with session name, agent type, hostname, detach hint, elapsed time, live viewer count (via MsgMeta push frames), connection state tracking, --status-top placement flag, and clean teardown. New `internal/statusbar` package with rune-safe width handling and terminal injection prevention. Phase 74 (multi-client fan-out) complete — relay Hub supports simultaneous WebSocket clients with per-client metadata, read-only mode, max-wins PTY resize arbitration, viewer count API, and CLI --readonly/--client flags. Codebase: ~16K Go (incl. 8K tests) + ~7K TS/TSX/CSS (incl. 2.6K tests). App runs as a tray-resident daemon with branded icons, splash screen, standard macOS menus, remote session discovery (GUI panel + CLI), auto-update notifications, guided Tailscale onboarding with auto-install, and collapsible left sidebar with Heroicons SVG icons — all navigation wired through sidebar. Settings is a sidebar tab (not modal) presenting all settings on a single scrollable page organized by labeled section headers (Appearance, Web Server, Paths) — no sub-tabs. Web server auto-starts on daemon launch — Tailscale mode when available, local network fallback (self-signed TLS + generated password) when not. New sessions are web-served by default. Terminal sessions support 138 curated xterm-theme color schemes (audited for WCAG-derived readability across all 4 agents) with live apply, localStorage persistence, and fallback guard for stale theme names. OpenCode sessions honor the selected theme via managed tui.json and SIGUSR2 broadcast for live switching. Settings tab provides web server URL actions (open in browser, copy to clipboard, QR code). Sidebar icons stay in fixed horizontal position during collapse/expand (48px icon slot via margin). Terminal content padded with 8px inset and dynamic background matching theme. All non-terminal GUI text meets WCAG AA 4.5:1 contrast ratio — muted text (#565f89) replaced with #9aa5ce across all app surfaces (tab bar, settings, welcome, modals, panels). System tray is fully cross-platform: macOS uses native cgo NSStatusBar, Linux uses D-Bus StatusNotifierItem protocol (GNOME/KDE/XFCE), Windows uses Shell_NotifyIcon Win32 API — all sharing menu helpers from tray_common.go. Daemon PATH augmentation discovers agent CLIs installed via cargo, snap, flatpak, npm, pnpm, and native installers across all platforms, plus Tailscale default install paths on Windows. Welcome screen macOS install command uses correct Homebrew tap + cask syntax.
...
---
*Last updated: 2026-04-15 after Phase 78 (tui-remote-qr) completion — v2.0 milestone complete*
