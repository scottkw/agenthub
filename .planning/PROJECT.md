...
## Last Shipped Milestone: v3.2 Plugin Suite (2026-05-12)
...
v3.2 Plugin Suite shipped 2026-05-12. 21 milestones shipped (v1.0–v3.2), 98 phases completed (numbering jumped from 90 → 92 because Phase 91 — v3.1 distribution-pipeline followups — is preserved under `.planning/deferred/91-distribution-pipeline-followups/` for a future cycle). 269 commits in v3.2 (~9-day shipping cycle). Closes GitHub Issues #35 (security hardening) and #36 (xterm.js plugin suite). The xterm.js core is now extended with 8 curated plugins, all vendored same-origin (no CDN, no remote fetches), surfaced behind a strict CSP (`script-src 'self'`, `'wasm-unsafe-eval'` only where the image addon's sixel decoder needs it), and surfaced in Settings → Plugins as 8 user-controlled toggles with inline disclosures for plugin-specific sub-config (Search defaults; Web-Links modifier/risk-policy; Image `storageLimit`). Capability-based session authorization (v3.1) plus byte-for-byte WS Origin allowlist (v3.1) plus vendored-only-no-CDN discipline (v3.1, extended in v3.2 via `vendor_drift_test.go` CI gate) is the security posture. Three access modes (GUI/CLI/TUI), multi-client fan-out with independent scrollback + max-wins resize, persistent DECSTBM status bar in CLI attach, Bubble Tea v2 TUI with full session lifecycle. Distribution via GitHub releases (DMG/EXE+NSIS/tar.gz+deb), Homebrew cask, WinGet, with release-please auto-versioning. 9 UAT items deferred to v3.3 (all blocked on raw shell session type — required to emit chafa sixel output, OSC 9;4 progress sequences, large scrollback paste, and IDN URL test data; some additionally require a physical iPad or a real Tailnet egress for parity verification). 6 polish-grade tech-debt items tracked for v3.2.1 or v3.3.
