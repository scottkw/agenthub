...
## Shipped Milestone: v4.2 Funnel Sharing & Polish — SHIPPED 2026-07-09

> ✅ Shipped 2026-07-09 (tag `v4.2`; merged to `main` via fast-forward). 13 phases (165–177), 61 plans, 45/45 requirements satisfied. Milestone audit PASSED (45/45 reqs, 0 integration blockers) — the one late gap (FNL-09 native-GUI FULL ACCESS exposure indicator, dropped at the app.go Wails bridge) was closed by gap-closure Phase 177 and confirmed live. Delivered public-internet session sharing over Tailscale Funnel (read-only reusable join code + consent-gated read/write with a single-use write code and colorblind-safe INTERNET/FULL ACCESS badges), a three-tab Share modal redesign, awaiting-input native notifications, and a large bug/dependency/platform-hardening sweep. Closes #107, #110, #112, #115, #116, #117, #118, #119, #120, #121, #123, #124, #125, #126, #127, #128, #129. **Override closeout** — accepted release-time live-UAT deferrals: M-37–M-40 (Funnel frontend), M-41 (win/linux notification delivery), M-45 (non-admin macsys), plus the #130 `top`-header known limitation. The scope below is retained as the milestone's design record.
...
- ✓ **v4.0 Hub-First Consolidation & UI/UX Overhaul (43 reqs)** — v4.0 (2026-06-23). NAV-01..05 (TUI + Sessions/Remote pages + New-Session item removed; sidebar → Home/Hub/Help/Settings), SHARE-01..06 (per-card Share modal, dual RO/RW cap issuance, file-browse derives from code), CARD-01..05 (header removal, origin + connected/available indicators, headless-VT mini-preview), TAB-01..04 (shrink-then-scroll tab strip + discoverability chevron), RDS-01..04 + CARRY-01..02 (v4.0 redesign: fonts/palette/radii, persisted Light/Dark, GroupSidebar ARIA), POL-01..05 (Hub/Settings polish, WebGL repaint hardening), TEST-01..06 (regression-test program + CI branch-protection gate), FIX-01..03 (#100 daemon race, #101 Windows files, #98 open-session), HELP-01 (#69 in-app Help), AGENT-01 (#65 Google Antigravity agent), SET-01 (#51 shell web-share warning toggle). ZOOM-01 cancelled (already shipped Phase 134; #99 closed). Full detail: `.planning/milestones/v4.0-REQUIREMENTS.md`.
- ✓ **v4.1 Session Chat (45 reqs)** — v4.1 (2026-06-29). CHAT-01..04 + IDENT-01/02 + ALIAS-UI-01/02 (per-session chat thread, tailnet-ID + settable alias from the shared sidebar), MENTION-01..03 (`@alias` autocomplete + one-way `@session`→PTY inject with confirm), PRESENCE-01/02 + PERSIST-01..03 (connected/typing presence, daemon-persisted JSONL thread with late-join scrollback + 10k cap + delete-on-session-delete), EXPORT-01 (Markdown export), NOTIF-01/02 (toggle + Hub-card unread/mention badges), PARITY-01 + WEBCHAT-01..06 + CHAT-LAYOUT-01/02 (cross-surface parity verified on the actually-shared link via `/sessions→/app/` redirect; scoped web guest; header-overflow + resizable width), SEC-01..03 + ROCHAT-01/02 + SEC-RO-01 (RO chat-send gate reversed to D-06 while inject/PTY stay RO-gated; sanitized inject; XSS-inert Markdown), VIEW-01..05 (#109 host-authority screen-share), INSTALL-01..03 + SETTINGS-UI-01 (#108) (Linux install.sh, winget id/repo fixes, Terminal-Plugins jump link), CHAT-FIX-01 + CHAT-PARITY-01 (toggle/Send overlap + terminal-tab chat). INSTALL-01 live install deferred to release-time. Full detail: `.planning/milestones/v4.1-REQUIREMENTS.md`.

### Active

No active milestone — v4.2 shipped 2026-07-09; next is set via `/gsd-new-milestone`. Open next-milestone candidates remain deferred: #122 (event-driven waiting-status), #111 (Warp-style UX), #93 (deferred #78 Hub fidelity), #49 (split panes), #10 (intersession orchestration), plus the carried release follow-up (push `main` to origin → run release-time live UATs: M-25 clean-box Linux install, M-37–M-40 Funnel frontend, M-41 win/linux notifications, M-45 non-admin macsys).
...
v4.2 Funnel Sharing & Polish shipped 2026-07-09 (tag `v4.2`; merged to `main` via fast-forward). 13 phases (165–177), 61 plans, 45/45 requirements satisfied, +57,470/−2,354 LOC across 545 files. AgentHub now shares a live session to the **public internet over Tailscale Funnel**: a recipient outside the tailnet joins with no Tailscale account, over browser-trusted TLS terminated at Tailscale's ingress, reaching AgentHub through a co-located plain-HTTP loopback hop (no second cert, no SNI selection). Public sharing spans **read-only** (a reusable, share-lifetime `/join?code=` join code, FNL-08) and **consent-gated read/write** ("FULL ACCESS" behind a press-and-hold typed acknowledgment, single-use write code, shorter expiry, threat-modeled, FNL-09) — both marked by colorblind-safe INTERNET / FULL ACCESS badges on the Hub card and session tab (the FULL ACCESS indicator's final app.go-bridge wiring gap was closed by gap-closure Phase 177 and confirmed live). The per-session Share modal was rebuilt as a three-tab segmented control (Tailnet·Private / Internet·Read-only / Internet·⚠ Full-access) that walls the public-write flow off entirely (#129). Awaiting-input native notifications landed (macOS `UNUserNotificationCenter`; Windows/Linux via beeep; edge-triggered, default OFF, #110), alongside a large sweep: web-guest plugin hot-swap over cap-gated SSE (#112), remote sessions open in-app tabs (#118), honest "Permission Limited" Tailscale detection on non-admin macOS (#120), owner disconnect-all-viewers + accurate remote-only viewer counts (#117/#121), mobile-readable guest viewport + remote disconnect banner + late-join terminal fidelity (#128/#125/#119), strict `/app/` CSP + Linux GUI hardening (#123/#124/#127), and Dependabot hygiene (7 low-risk merged / 3 high-risk deferred with ignore rules). Milestone audit PASSED (45/45 reqs, 0 integration blockers, 13/13 phases verified); **override closeout** — accepted release-time live-UAT deferrals (M-37–M-40 Funnel frontend, M-41 Windows/Linux notification delivery, M-45 non-admin macsys) and the #130 macOS `top`-header known limitation. Colorblind-safe + `prefers-reduced-motion` remain release norms; cross-surface (GUI/CLI/web) parity remains release-blocking. Carry-forward operator pre-release tokens (RELEASE_PUBLISH_TOKEN, WINGET_FIRST_SUBMISSION) still required before the next tagged binary release; `main` is now ~478 commits ahead of origin/main (unpushed until release).

<details>
<summary>Prior Current State — v4.1 Session Chat (2026-06-29)</summary>

v4.1 Session Chat shipped 2026-06-29 (tag `v4.1`, local-unpushed until release per the v3.6/v4.0 pattern). 14 phases (151–164), 54 plans, 45/45 requirements, 381 commits (+51,309/−2,306 LOC across 532 files) over a 4-day cycle (2026-06-25 → 2026-06-28). AgentHub now carries a **per-session human-to-human chat side-channel** that works identically on the desktop GUI session tab, the Hub interactive modal, and the web-share browser guest — parity by construction, since all three mount the same `ChatPanel`. The daemon persists each thread as JSONL (`~/.config/agenthub/chats/<id>.jsonl`, 10k cap, late-join scrollback, dies with the session) and serves Markdown export. Participants are identified by tailnet ID + a self-chosen alias settable live from the chat sidebar (`AliasStore`/`MsgAliasSet`). `@alias` autocompletes participants; **`@session` injects one-way into the agent's PTY** (sanitized, write-cap-gated, press-and-hold confirm). Real web-share parity was finally closed by redirecting the actually-shared `/sessions/{id}?cap=` link to the chat-capable `/app/` SPA (Phase 155 had verified parity only on a surface no remote guest reached); scoped web guests see terminal + chat + cap-gated file tab and no app chrome. Read-only guests are full chat participants (D-06 reversed the SEC-01 chat-send gate) while `@session` inject and raw PTY input stay byte-for-byte RO-gated (security re-reviewed, `threats_open: 0`). Orthogonal wins: Issue #109 host-authority screen-share semantics (guests honor a single server-pushed grid + CSS scale-to-fit, web/desktop parity), a working Linux `scripts/install.sh` + corrected winget id/repo strings, and the #108 Terminal-Plugins jump-link fix. Markdown rendering is XSS-inert (`remark-gfm` only, no raw HTML). Milestone audit PASSED (45/45 reqs, 14/14 phases + integration, 6/6 flows); **override closeout** with one accepted deferral — the M-25 clean-box Linux install runs as a release-time step after `main` (currently ~381 commits ahead of origin) is pushed. Colorblind-safe + `prefers-reduced-motion` remain release norms; cross-surface (GUI/CLI/web) parity remains release-blocking. Carry-forward operator pre-release tokens (RELEASE_PUBLISH_TOKEN, WINGET_FIRST_SUBMISSION) still required before the next tagged binary release.

</details>

<details>
<summary>Prior Current State — v4.0 Hub-First Consolidation & UI/UX Overhaul (2026-06-23)</summary>

v4.0 Hub-First Consolidation & UI/UX Overhaul shipped 2026-06-23 (tag `v4.0`). 15 phases (136–150; 151 cancelled), 52 plans, 43/43 active requirements (NAV/SHARE/CARD/TAB/RDS/CARRY/POL/TEST/FIX/HELP/AGENT/SET), 407 files changed (+58K/−17K) across a 5-day cycle (2026-06-19 → 2026-06-23). AgentHub is now **Hub-first**: the TUI surface, the Sessions page (`DaemonManagerPanel`), the Remote page (`RemoteSessionsPanel`), and the sidebar New-Session item are all removed; cross-surface parity narrows to **GUI/CLI/web**. Session creation flows solely through the Hub's `HubFilterBar`; the sidebar is Home / Hub / Help / Settings. A single per-card **Share** modal mints RO + RW capability tokens and gates remote file browsing (browse permission derives from the presented code); remote "Open in browser" reuses the held web-share cap. The v4.0 visual redesign is adopted (Plus Jakarta Sans + JetBrains Mono, comp palette/radii/type-scale, a persisted Light/Dark toggle) after catching and correcting a token-only false pass in Phase 141. New surfaces: a Chrome-style shrink-then-scroll tab strip with a discoverability chevron, an in-app Help page (searchable docs + external links, #69), and a Google Antigravity (`agy`) agent wired across all surfaces with a `#ff9e64` badge identity (#65, verified by live REPL UAT). A formal regression-test program landed (TESTING.md suite manifest + requirement→test traceability + path-check CI script + 17-item manual checklist) with branch protection on `main` (4 build jobs + playwright required). Milestone audit PASSED: 43/43 requirements satisfied, 5/5 E2E flows wired, all 15 phases Nyquist-compliant, zero open gaps. Colorblind-safe + `prefers-reduced-motion` remain release norms; cross-surface (GUI/CLI/web) parity remains release-blocking. Carry-forward operator pre-release tokens (RELEASE_PUBLISH_TOKEN, WINGET_FIRST_SUBMISSION) still required before the next tagged binary release.

</details>

<details>
<summary>Prior Current State — v3.6 Hub (Session Grid / Control Room) (2026-06-19)</summary>
...
| Drop the TUI surface in v4.0 (parity → GUI/CLI/web) | TUI Hub parity was an open carry (#82); maintaining a 4th surface for a single-user desktop app wasn't worth the cost | ✓ Good — `internal/tui` + `tui` subcommand deleted; cross-surface parity contract narrows to GUI/CLI/web |
| Hub-first wins over the redesign comps | The Claude redesign mockups still showed Sessions/Remote pages; the structural Hub-first removals are the real intent | ✓ Good — visual language adapted onto the Hub-first structure surface-by-surface at UI-spec time |
| One per-card Share modal mints both RO + RW cap-tokens; file-browse derives from the presented code | Couples the capability model into a single share action (security-sensitive); avoids a separate file-browse cap flow | ✓ Good — isolated for review in Phase 137; remote "Open in browser" reuses the held cap (no second single-use code) |
| Render Hub tail/preview through a headless VT emulator (not regex ANSI strip) | Regex stripping garbled agent TUI output (#96); a real VT emulator preserves spacing and absorbs private-CSI escapes | ✓ Good — `GetSessionStyledTailLines`; required a query-sequence pre-strip to kill a `go test -race` data race (#100, Phase 144) |
| Render-compare app-vs-comp gate (not token/hex/ARIA checks alone) | Phase 141's first pass was a FALSE PASS — tokens were recolored to the same values; the comp fonts/palette were never adopted | ⚠️ Lesson — source-level gates can certify a no-op redesign; re-verification must render the running app against the canonical comp |
| Path-aware shell detection for the web-share warning (`isShellCli`/`isShellSession`) | The bare-name gate never fired — a shell session's cli is its full path (`/bin/zsh`); green tests encoded the same wrong assumption | ⚠️ Lesson — found in live UAT (Phase 150); shared `lib/shellCli.ts` normalizes basenames; drive UATs with real daemon data |
| Store-before-transport-before-UI phasing for chat (v4.1) | The JSONL schema is the wire contract everything serializes to; late schema changes cascade through relay + both UI surfaces | ✓ Good — `ChatStore`/`ChatMessage` (Phase 151) landed before relay frames (152) before the shared `ChatPanel` (154/155) |
| One shared `ChatPanel` for all three surfaces (GUI tab, Hub modal, web-share guest) | Cross-surface parity by construction — a single un-forked component means a fix lands everywhere at once | ✓ Good — alias control, RO-can-post, and layout polish each shipped once and applied to all three |
| `@session` is a one-way PTY bridge only (no PTY→message parsing) | Segmenting an agent's TUI paint-stream into discrete chat messages is the hard problem discovery explicitly designed out | ✓ Good — agent-as-chat-author stayed out of scope; inject is sanitized, write-cap-gated, confirm-gestured |
| Redirect the actually-shared `/sessions/{id}?cap=` → `/app/` SPA (Phase 159) | Phase 155 verified PARITY-01 only on `/app/`, a surface no remote guest was ever sent to — false parity; the raw `terminal.js` viewer drops chat frames | ✓ Good — closed the real gap with a server-side 302 (after cap check); also pre-unblocks the Funnel milestone |
| Reverse the SEC-01 RO chat-send gate to D-06; keep inject/PTY RO-gated (Phase 163) | D-06 ("RO clients are full chat participants") and SEC-01 ("RO cannot post") conflicted; user ruled RO can chat but not `@session`-inject | ✓ Good — only `HandleChatSend` loosened; `HandleInject`/`MsgInput` byte-for-byte intact, security re-reviewed (`threats_open: 0`) |
| Host-authority PTY arbiter for screen-share (#109, Phase 157) replacing max-wins | Max-wins thrashed the grid across viewers and fought the single-source-of-truth model; guests should conform + CSS scale-to-fit | ✓ Good — VIEW-01..05; web + desktop guests honor one server-pushed grid, never drive the PTY |
...
---
*Last updated: 2026-07-09 — v4.2 Funnel Sharing & Polish milestone SHIPPED (tag `v4.2`; merged to `main` via fast-forward). 13 phases (165–177), 61 plans, 45/45 requirements, +57,470/−2,354 LOC across 545 files. Delivered public-internet session sharing over Tailscale Funnel — read-only reusable `/join?code=` join code (FNL-08) + consent-gated read/write "FULL ACCESS" behind a hold-to-confirm typed acknowledgment with a single-use write code and colorblind-safe INTERNET / FULL ACCESS badges (FNL-09; the final app.go-bridge wiring gap closed by gap-closure Phase 177) — plus a three-tab Share modal redesign (#129), awaiting-input native notifications (#110), and a large bug/dependency/platform-hardening sweep. Milestone audit PASSED (45/45 reqs, 0 integration blockers, 13/13 phases verified); **override closeout** — accepted release-time live-UAT deferrals (M-37–M-40, M-41, M-45) + the #130 macOS `top`-header known limitation. Closes #107, #110, #112, #115, #116, #117, #118, #119, #120, #121, #123, #124, #125, #126, #127, #128, #129. `main` is now ~478 commits ahead of origin/main (unpushed until release). No active milestone — next is set via `/gsd-new-milestone`.*
