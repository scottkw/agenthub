...
**v4.2 — Funnel Sharing & Polish** (unreleased — in development on the `v4.2-funnel-sharing` branch) — turns AgentHub into a full public-sharing surface over Tailscale Funnel (read-only *and* consent-gated read/write), adds native awaiting-input notifications, redesigns the Share modal, and sweeps a large round of bug fixes, dependency updates, and platform hardening.

- **One-click public sharing over Tailscale Funnel** ([#107](https://github.com/scottkw/agenthub/issues/107)) — Share a session to the public internet straight from the per-session Share modal — the recipient needs no Tailscale account and gets browser-trusted TLS terminated by Tailscale's ingress. The flow is risk-gated (a two-step enable behind an explicit risk panel), reversible in one click, and auto-expiring. Guests reach the session via a co-located plain-HTTP loopback hop behind Tailscale's single public TLS termination (no second certificate, no SNI juggling). A colorblind-safe globe + **INTERNET** badge marks any publicly-exposed session on its Hub card and tab.
- **Reusable public read-only join code** — the public read-only link carries a **reusable, share-lifetime join code** (not a 5-minute single-use tailnet code), so a shared URL keeps working for every guest until you disable the share or it auto-expires.
- **Consent-gated public read/write (FULL ACCESS)** — public *write* access is reachable only behind a distinct hard consent gate (typed/press-and-hold acknowledgment), mints a **single-use** write code with a shorter default/max expiry than read shares, and is covered by a threat model. A colorblind-safe **FULL ACCESS** badge (text label + non-color unlock icon) marks a write-exposed session on its Hub card and tab, alongside the INTERNET badge.
- **Redesigned Share modal** ([#129](https://github.com/scottkw/agenthub/issues/129)) — a fixed control strip plus a three-tab segmented panel (**Tailnet · Private** / **Internet · Read-only** / **Internet · ⚠ Full-access**) whose surfaces *swap* instead of stacking, with the public-write/command-exec flow walled off entirely in the Full-access tab and keyboard-navigable (WAI-ARIA roving-tabindex) tabs.
- **Awaiting-input native notifications** ([#110](https://github.com/scottkw/agenthub/issues/110)) — Optional native OS notifications (macOS `UNUserNotificationCenter`; Windows/Linux via beeep) fire once when a session transitions into the waiting-for-input state while the app is hidden to the tray. Edge-triggered (exactly one notification per transition, de-duplicated), default **OFF**, toggled in Settings → Behavior.
- **In-app sharing guide** — A searchable Help article walks through the Funnel share flow and the contained device-share + ACL alternative.
- **Web-share, remote-viewer & windowing fixes** ([#119](https://github.com/scottkw/agenthub/issues/119), [#125](https://github.com/scottkw/agenthub/issues/125), [#126](https://github.com/scottkw/agenthub/issues/126), [#128](https://github.com/scottkw/agenthub/issues/128)) — mobile web-share guests get a readable downscaled viewport instead of a microscopic one; remote viewers see a colorblind-safe disconnect banner when the owner ends or disconnects them; and late-joining guests reconnect to a byte-faithful terminal snapshot (agent TUIs render clean after scrollback ring-wrap).
- **Settings & bug-fix polish** ([#112](https://github.com/scottkw/agenthub/issues/112), [#115](https://github.com/scottkw/agenthub/issues/115), [#116](https://github.com/scottkw/agenthub/issues/116), [#117](https://github.com/scottkw/agenthub/issues/117), [#118](https://github.com/scottkw/agenthub/issues/118), [#120](https://github.com/scottkw/agenthub/issues/120), [#121](https://github.com/scottkw/agenthub/issues/121)) — web-guest plugin hot-swap over the capability-gated SSE stream; the footer "Share Session" pill tracks the Share modal's live state; a "Stay on Hub after creating a session" toggle; owner-driven "disconnect all viewers" plus accurate remote-only viewer counts; remote tailnet sessions open in an in-app terminal tab instead of an external browser; and honest **"Permission Limited"** Tailscale detection on non-admin macOS accounts where the `macsys` status file is unreadable — instead of a misleading "not connected".
- **Platform hardening** ([#123](https://github.com/scottkw/agenthub/issues/123), [#124](https://github.com/scottkw/agenthub/issues/124), [#127](https://github.com/scottkw/agenthub/issues/127)) — the guest `/app/` SPA route now carries the same strict Content-Security-Policy as the other web surfaces (hashed assets stay cacheable); the desktop GUI's role menus and DMABUF webview are guarded so the Linux build launches cleanly; and the Hub-card mini-preview wrapping is refined.
- **Hub-card refinement & dependency hygiene** — the Hub session card's metadata consolidates into one quiet chip row with the exposure badge kept deliberately prominent (a security signal that must stay unmissable); seven low-risk Dependabot updates were merged behind the full gate and three high-risk ones (Wails, tailscale.com, actions/checkout major) deferred with documented ignore rules.

Will close [#107](https://github.com/scottkw/agenthub/issues/107), [#110](https://github.com/scottkw/agenthub/issues/110), [#112](https://github.com/scottkw/agenthub/issues/112), [#115](https://github.com/scottkw/agenthub/issues/115), [#116](https://github.com/scottkw/agenthub/issues/116), [#117](https://github.com/scottkw/agenthub/issues/117), [#118](https://github.com/scottkw/agenthub/issues/118), [#119](https://github.com/scottkw/agenthub/issues/119), [#120](https://github.com/scottkw/agenthub/issues/120), [#121](https://github.com/scottkw/agenthub/issues/121), [#123](https://github.com/scottkw/agenthub/issues/123), [#124](https://github.com/scottkw/agenthub/issues/124), [#125](https://github.com/scottkw/agenthub/issues/125), [#126](https://github.com/scottkw/agenthub/issues/126), [#127](https://github.com/scottkw/agenthub/issues/127), [#128](https://github.com/scottkw/agenthub/issues/128), [#129](https://github.com/scottkw/agenthub/issues/129). _Release notes and download links will be added when v4.2 ships._

### Previous Releases

Full notes for every release live on the **[GitHub Releases page](https://github.com/scottkw/agenthub/releases)**.

- **[v4.1 — Session Chat](https://github.com/scottkw/agenthub/releases/tag/v4.1)** (2026-06-28) — a per-session, human-to-human chat side channel with an `@session` agent bridge, cross-surface parity (GUI / Hub modal / web-share), read-only-guest participation, and terminal screen-share fidelity ([#109](https://github.com/scottkw/agenthub/issues/109)).
- **[v4.0 — Hub-First Consolidation & UI/UX Overhaul](https://github.com/scottkw/agenthub/releases/tag/v4.0)** (2026-06-23) — collapses AgentHub onto a single Hub-centric surface, retires the TUI, ships the v4.0 visual redesign (Plus Jakarta Sans / JetBrains Mono, Light/Dark theme), and stands up a formal regression-test program.
- **[v3.5.1 — Remote Browse Completion + Release-Gate Fix](https://github.com/scottkw/agenthub/releases/tag/v3.5.1)** (2026-06-16) — completes the desktop-GUI remote-browse on-ramp (discover → list → pick → browse a tailnet peer's files) and clears the flaky release gate, retiring the remote file-browser epic.
- **[v3.5 — File Browser: Write Operations & Editor](https://github.com/scottkw/agenthub/releases/tag/v3.5)** (2026-06-15) — create, edit, delete, rename, and upload files from any session, plus an in-app CodeMirror editor, across desktop, web-share, and TUI.
- **[v3.4.2 — Dependency maintenance](https://github.com/scottkw/agenthub/releases/tag/v3.4.2)** (2026-06-14) — a patch release consolidating six Dependabot updates; no user-visible behavior change.
- **[v3.4.1 — Test reliability](https://github.com/scottkw/agenthub/releases/tag/v3.4.1)** (2026-06-14) — deflakes a class of hub subscriber/viewer-count CI tests; no user-visible behavior change.
- **[v3.4 — File Browser (Read-Only) + TUI Parity](https://github.com/scottkw/agenthub/releases/tag/v3.4)** (2026-06-13) — browse, preview, and download files from any session across all three surfaces, including remote tailnet sessions.
- **[v3.3.2 — Dependency maintenance](https://github.com/scottkw/agenthub/releases/tag/v3.3.2)** (2026-05-19) — a patch release rolling forward dependencies; no user-visible behavior change.
- **[v3.3.1 — Bug Sweep](https://github.com/scottkw/agenthub/releases/tag/v3.3.1)** (2026-05-19) — Windows daemon named-pipe IPC, OSC color-query / DA1 leak fixes on both relay surfaces, WebGL recovery banner, iPad touch-scroll, and Linux shell auto-close.

Older releases (v3.3 and earlier) are on the [Releases page](https://github.com/scottkw/agenthub/releases).
...
- **Per-card Share modal** — Toggles per card: *Share the session* (reveals read-only + read/write capability links) and *Enable remote file browsing* (permission derives from the presented share code), plus a risk-gated *Share to the public internet* Funnel toggle (v4.2) that surfaces a read-only public URL with copy/open/QR actions and one-click disable. A re-enableable warning fires before web-sharing a raw shell session
...
### Notifications (v4.2)
- **Awaiting-input alerts** — When a session transitions into the waiting-for-input state while AgentHub is hidden to the tray, you get a native OS notification so you don't have to watch the tray. Native macOS `UNUserNotificationCenter` on macOS; beeep-backed notifications on Windows and Linux
- **Edge-triggered & de-duplicated** — Exactly one notification per non-waiting→waiting transition (cold-start baseline suppressed, per-session de-dup), so a session sitting at a prompt never re-nags
- **Opt-in** — Default **OFF**; enable with the "Notify when a session needs input" toggle in Settings → Behavior
...
- **Public sharing over Tailscale Funnel (v4.2)** — Expose a single session to the public internet from the per-session Share modal — the recipient needs no Tailscale account and gets browser-trusted TLS terminated at Tailscale's ingress. Enabling is a deliberate two-step behind a risk panel; it is reversible in one click and auto-expires. Public TLS is terminated once by Tailscale; AgentHub serves the guest over a co-located plain-HTTP loopback hop (no second certificate or SNI selection). Any publicly-exposed session shows a colorblind-safe globe + **INTERNET** indicator on its Hub card and tab; when Tailscale is in local-fallback mode the Funnel toggle fails closed
...
- **Behavior section** — "Start minimized to system tray" toggle; a "Stay on Hub after creating a session" toggle (v4.2) that keeps you on the Hub instead of auto-switching to the new session's tab; and a "Notify when a session needs input" toggle (v4.2, default off) for native awaiting-input notifications — each with non-optimistic save, loading state, and error feedback
...
- **Tailscale status indicator** — Color-coded dot with a text label (Connected / Permission Limited / Not Connected / Daemon Stopped / Not Installed) and a collapsible diagnostics checklist showing binary detection, daemon status, connection state, and TLS readiness; platform-specific troubleshooting instructions for macOS, Linux, and Windows. The **Permission Limited** state (v4.2) honestly distinguishes "the daemon is running but this non-admin macOS account can't read its status file" from "not connected", with guidance to grant admin access or install the Homebrew build — instead of silently misreporting a live daemon as disconnected ([#120](https://github.com/scottkw/agenthub/issues/120))
