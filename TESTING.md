...
| vitest (frontend) | **130** `*.test.ts/tsx` files | `frontend/src/` | `cd frontend && pnpm test` | React component render contracts, UI state, CSS token source gates, lib adapters (relay, remote, hub, status) |
...
| WEBCHAT-05 | frontend/e2e/web-share-scope.spec.ts | Playwright | Phase 159-04 (gap-closure): a web-share guest cannot rename the session tab — no tab-chevron / "Session menu" button and double-click does not open the rename input. Guards the TabBar `webMode` prop that suppresses Rename/Save-Terminal-As (Wails-only) for guests. |
